...
setwd("C:/MyRCode/Seoul_food_analytics-")
# 폴더명 생성
...
sum(which(is.na(year_18_food_data_frame$시도)) != which(!is.na(year_18_food_data_frame$발신지_시도)))

# 시군구 데이터의 na index와, 발신지_구의 na가 아닌 곳의 index를 확인해보기
sum(which(is.na(year_18_food_data_frame$시군구)) != which(!is.na(year_18_food_data_frame$발신지_구)))

# 읍면동 데이터의 na index와, 발신지_동의 na가 아닌 곳의 index를 확인해보기
sum(which(is.na(year_18_food_data_frame$읍면동)) != which(!is.na(year_18_food_data_frame$발신지_동)))
...
# 기준일
sum(which(is.na(year_18_food_data_frame$기준일)) != which(!is.na(year_18_food_data_frame$일자)))
year_18_food_data_frame$기준일[which(is.na(year_18_food_data_frame$기준일))] <- year_18_food_data_frame$일자[which(!is.na(year_18_food_data_frame$일자))]
sum(is.na(year_18_food_data_frame$기준일))

# 연령대
sum(which(is.na(year_18_food_data_frame$연령대)) != which(!is.na(year_18_food_data_frame$연령)))
year_18_food_data_frame$연령대[which(is.na(year_18_food_data_frame$연령대))] <- year_18_food_data_frame$연령[which(!is.na(year_18_food_data_frame$연령))]
